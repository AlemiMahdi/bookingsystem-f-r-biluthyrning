...
/* ═══════════════════════════════════════════
   GEMENSAM EDIT / DELETE FÖR ADMIN-TABELLER
   Används av:
   - admin-bookings.js
   - admin-users.js
   - admin-cars.js
...
function bindEditDeleteActions({
  tableId,
  items,
  editSelector,
  deleteSelector,
  ...
  if (table.dataset.editDeleteBound === 'true') return;
  table.dataset.editDeleteBound = 'true';

  table.addEventListener('click', async e => {
    const deleteBtn = e.target.closest(deleteSelector);
    const editBtn = e.target.closest(editSelector);

    if (deleteBtn && table.contains(deleteBtn)) {
      e.preventDefault();

      const id = deleteBtn.dataset[deleteDatasetKey];

      if (!confirm(confirmMessage)) return;

      try {
        const res = await apiFetch(deleteUrl(id), {
          method: 'DELETE',
        });
    ...
    if (editBtn && table.contains(editBtn)) {
      ...
      const id = Number(editBtn.dataset[editDatasetKey]);
